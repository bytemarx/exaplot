 ...
 * may be a 'PathLike' object or a callable with no arguments
 * that returns a 'PathLike' object.
 * 
...
{
    auto pyOwned_datafile = PyObject_CallNoArgs(pyBorrowed_datafileFn);
    if (pyOwned_datafile == NULL)
        return Error::pyerror(Error::DATAFILE);

    auto status = convertDatafilePath(pyOwned_datafile, datafile);
